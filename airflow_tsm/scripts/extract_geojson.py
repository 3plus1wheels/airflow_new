...
def worker_tif_to_geojson(args):
    ...
    Lấy giá trị depth trung bình theo từng block SAMPLE_RATE x SAMPLE_RATE.
    """
        ...
        with rasterio.open(input_tif) as src:
            ...
            for row in range(0, src.height, SAMPLE_RATE):
                for col in range(0, src.width, SAMPLE_RATE):
                    ...
                    # Lọc giá trị hợp lệ trong block
                    if nodata is None:
                    ...
                    # Nếu block không có giá trị hợp lệ thì bỏ qua
                    if valid_depths.size == 0:
                        continue

                    # Lấy depth trung bình của block
                    avg_depth = float(valid_depths.mean())

                    # Tính polygon cho cả block
                    left, top = transform * (col, row)

                    # Chặn không vượt quá biên raster
                    ...
                    cells.append(
                        {
                            "geometry": poly,
                            "properties": {"depth": round(avg_depth, 2)},
                        }
                    )
...
def run_extract_geojson(input_dir, output_dir):
    ...
    print(
        f"⚙️ Config: Sample Rate={SAMPLE_RATE}, Min Depth={MIN_DEPTH_THRESHOLD}m, Processes={MAX_WORKERS}"
    )
    ...
    with multiprocessing.Pool(processes=MAX_WORKERS, maxtasksperchild=1) as pool:
        for res in pool.imap_unordered(worker_tif_to_geojson, tasks, chunksize=1):
            print(res)
